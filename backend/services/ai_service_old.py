...
class AIService:
    ...
    async def call_deepseek_api(self, prompt: str, **kwargs) -> AIResponse:
        """调用DeepSeek API"""

        # 检查API密钥
        if not AIConfig.DEEPSEEK_API_KEY:
            print("警告：未配置DEEPSEEK_API_KEY，使用模拟回复")
...
1. **整体评价**：先判断学生回答的答案对不对，给出学生回答的总体评价，指出回答的准确性和完整性

2. **回答亮点分析**：首先指出学生回答中的优秀之处、正确理解的部分，给予积极鼓励

3. **知识掌握评估**：分析学生对该知识点的理解程度，是否抓住了核心概念

4. **不足之处指正**：具体指出回答中的错误、遗漏或不够准确的地方

5. **改进建议**：提供具体的学习建议，告诉学生如何提高和完善答案

6. **知识拓展**：补充相关的重要知识点，帮助学生建立更完整的知识体系

7. **学习指导**：给出后续学习的方向和建议

8. **综合评分**：给出1-10分的评分（10分为满分），并详细说明评分理由
